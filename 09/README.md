...
out where _not_ to step.

Consider a rope with a knot at each end; these knots mark the _head_ and
the _tail_ of the rope. If the head moves far enough away from the tail,
...
target="_blank" rel="noopener">Planck lengths</a>, you should be able to
model the positions of the knots on a two-dimensional grid. Then, by
following a hypothetical _series of motions_ (your puzzle input) for the
...
fact, the head (`H`) and tail (`T`) must _always be touching_
...
steps, then _left_ three steps, then _down_ one step, and so on. After
...
_tail visited at least once_. In this diagram, `s` again marks the
...
Simulate your complete hypothetical series of motions. _How many
positions does the tail of the rope visit at least once?_
...
Rather than two knots, you now must simulate a rope consisting of _ten_
...
position. However, _be careful_: more types of motion are possible than
...
knots. _How many positions does the tail of the rope visit at least
once?_
